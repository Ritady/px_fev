...
tamperInfo = {"Name":'tamper', "Enable":'', "Num":''}
...
def generate_iocfg_macro_config_h(fileObj):
    ...
    tamperArry = get_io_config_info(tamperInfo,fileObj,fileJs)
    ...
    set_io_iniput_output_array(fileObj,tamperArry,"TAMPER_IO_LIST")
...
def get_io_base_info():
    ...
    global ledInfo, keyInfo, sensorInfo, tamperInfo
    file = open('package.json', 'rb')
    fileJson = json.load(file, object_pairs_hook=collections.OrderedDict)
    ...
    tamperInfo["Enable"] = ioConfigJson['tamper_enable'] if 'tamper_enable' in ioConfigJson else "false"
    tamperInfo["Num"] = ioConfigJson['tamper_num'] if 'tamper_enable' in ioConfigJson else "false"
    file.close()
    return fileJson
