...
if %chip% == EFR32MG13P732F512 (
  %IAR_PROJECT_DIR%\..\..\..\tools\iar_tools\commander.exe gbl create "%targetdir%/sdk_route.gbl" --app "%targetdir%/sdk_route.s37" --device %chip%
) else (
  %IAR_PROJECT_DIR%\..\..\..\tools\iar_tools\commander.exe convert "%targetdir%/sdk_route.s37" --secureboot --keyfile %IAR_PROJECT_DIR%\..\..\..\tools\iar_tools\signing-key --outfile "%targetdir%/sdk_route_security.s37"
  %IAR_PROJECT_DIR%\..\..\..\tools\iar_tools\commander.exe gbl create "%targetdir%/sdk_route.gbl" --app "%targetdir%/sdk_route_security.s37" --sign %IAR_PROJECT_DIR%\..\..\..\tools\iar_tools\signing-key --encrypt %IAR_PROJECT_DIR%\..\..\..\tools\iar_tools\encryption-key --device %chip%
) 
::cd D:\zigbee_project\sdk_router\app\builder\sdk_route_user\tools\
...
if %chip% == EFR32MG13P732F512 (
  ...
  %IAR_PROJECT_DIR%\..\..\..\tools\iar_tools\commander.exe convert %IAR_PROJECT_DIR%\..\..\..\tools\iar_tools\%chip%_bootloader.s37 %targetdir%\sdk_route_security.s37 --address 0x0 --outfile %targetdir%\%NAME%_QIO_%STR_VER%.s37
) 
