...
typedef enum {
    ...
    NET_LOST,               ///< network lost, lost parent
...
typedef enum {
    ...
    MSG_SRC_GW_MULTICAST,   ///< Gateway multicast address
    MSG_SRC_DEV_UNICAST,    ///< device unicast address
    MSG_SRC_DEV_BROADCAST,  ///< device broadcast address
    MSG_SRC_DEV_MULTICAST,  ///< device multicast address
}MSG_SRC_T;
...
#ifdef MCU_CORE_8258
    uint8_t manu_code_enable; //1: for enable manufacturer code
...
/**
 * @note change poll failed times (ms)
 * @param[in] {times} failed times
 * @return none
 */
extern void zg_poll_failed_times_change(uint16_t times);
