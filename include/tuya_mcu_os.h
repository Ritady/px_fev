...
typedef enum {
    ...
    CALLBACK_TYPE_TIME_SYNC,          ///< after time sync
}CALLBACK_TYPE_T;
...
typedef enum {
    QUEUE_RET_OK = 0, ///< Display voltage reported to the gateway
    QUEUE_RET_ERR,    ///< Display voltage reported to the gateway
}QUEUE_RET_T;
...
///@name Get SDK information
/**
 * @note Check whether the tuya SDK has been authenticated and verified.
 * @param none
 * @return bool_t: 
 *  TRUE means that the SDK is authenticated, you can use it without limit.
 *  FALSE means that the SDK can only be used for 7 days.
 */
bool_t tuya_sdk_verification_is_ok(void);

///@name queue tools
extern dev_queue_t create_queue(uint8_t size);
extern QUEUE_RET_T push_queue(dev_queue_t q, void *data);
extern void *pop_queue(dev_queue_t q);
extern void *get_queue(dev_queue_t q);
extern uint8_t get_queue_len(dev_queue_t q);
