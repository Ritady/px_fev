...
#ifdef MCU_CORE_8258
typedef enum {
    GPIO_IRQ_SRC_RISC0 = 2,
    GPIO_IRQ_SRC_RISC1,
} GPIO_IRQ_SRC_E;

typedef struct {
    GPIO_IRQ_SRC_E irq_mode;
    GPIO_PORT_T port;
    GPIO_PIN_T pin;
} dev_gpio_int_cfg_t;

/**
 * @note Configure interrupt source of device
 * @param[in] {cfg} interrupt source list
 * @param[in] {cfg_sums} cfg list sums
 * @return TRUE for success, False for failure 
 */
extern bool_t dev_gpio_int_src_cfg(dev_gpio_int_cfg_t *cfg, uint8_t cfg_sums);
#endif
