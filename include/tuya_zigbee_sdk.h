...
#define APP_DEBUG 1
...
#ifndef MCU_CORE_8258
    #define sdk_malloc(x) malloc(x)
    #define sdk_free(x) free(x)
    /**
    ...
    #endif
#else 
    #define malloc(x) sdk_malloc(x)
    #define free(x) sdk_free(x)
    #if APP_DEBUG
        #define app_print(...)     Tl_printf(__VA_ARGS__)
        #define APP_PRINT(fmt,...) Tl_printf("%s(%d)-<%s>: "##fmt"\r\n",__FILE__, __LINE__, __FUNCTION__,##__VA_ARGS__)
    #else
        #define app_print(...)
        #define APP_PRINT(...)
    #endif
 ...
 * @return none
 */
VIRTUAL_FUNC bool_t key_check_use_poll(void);
